...
function get_species_info!(lines, cri::Chem_rxn_info)
  cri.num_species = 1
  for i in 1:length(lines)
    ...
    if contains(lines[i],"@species")
      while !contains(lines[i+cri.num_species],"@")
        ...
        init_x_str = lines[i+cri.num_species][(eq_ind+1):end]
      ...
      cri.num_species = cri.num_species - 1 #corrects an off-by-one
    end
  end
end
